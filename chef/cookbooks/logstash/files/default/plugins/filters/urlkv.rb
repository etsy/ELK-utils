...
class LogStash::Filters::URLKV < LogStash::Filters::Base
  config_name "urlkv"
  ...
  def register
    ...
    @scan_re = Regexp.new("((?:\\\\ |[^"+@field_split+@value_split+"\\W])+)["+@value_split+"](?:\"([^\"]+)\"|'([^']+)'|((?:\\\\ |[^"+@field_split+"])+))")
  end # def register
...
end # class LogStash::Filters::URLKV
